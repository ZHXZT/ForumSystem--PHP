...
if (isset($_POST['submit'])){
    ...
    $query="select * from xzt_son_module where module_name='{$_POST['module_name']}'and id!={$_GET['id']}";
    $result=execute($link,$query);
    if (mysqli_num_rows($result)){
        skip('son_module_add.php','error','子版块已经存在！');
    }

    $query="update xzt_son_module set father_module_id={$_POST['father_module_id']},module_name='{$_POST['module_name']}',info='{$_POST['info']}',member_id={$_POST['member_id']},sort={$_POST['sort']} where id={$_GET['id']}";
    execute($link,$query);
    if (mysqli_affected_rows($link)==1){
        skip('son_module.php','ok','修改成功');
    }else{
        skip('son_module.php','error','修改失败请重试');
    }
}
?>
...
<?php include "inc/header.inc.php";?>
    ...
    <div id="main">
        <div class="title" style="margin-bottom: 20px">编辑子板块 - <?php echo $data['module_name']?></div>
        <form method="post">
            <table class="au">
                <tr>
                    <td>所属板块</td>
