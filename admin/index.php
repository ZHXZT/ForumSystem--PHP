...
$query="select * from xzt_manage where id={$_SESSION['manage']['id']}";
$result_manage=execute($link,$query);
$data_manage=mysqli_fetch_assoc($result_manage);

$query="select count(*) from xzt_father_module";
$count_father_module=num($link,$query);

$query="select count(*) from xzt_son_module";
$count_son_module=num($link,$query);

$query="select count(*) from xzt_content";
$count_content=num($link,$query);

$query="select count(*) from xzt_reply";
$count_reply=num($link,$query);

$query="select count(*) from xzt_member";
$count_member=num($link,$query);

$query="select count(*) from xzt_manage";
...
<div id="main">
    ...
    <div class="explain">
        <ul>
            <li>|- 父版块(<?php echo $count_father_module?>)  子版块(<?php echo $count_son_module?>)  帖子(<?php echo $count_content?>)  回复(<?php echo $count_reply?>)  会员(<?php echo $count_member?>)  管理员(<?php echo $count_manage?>)</li>
        </ul>
    </div>
    <div class="explain">
        <ul>
